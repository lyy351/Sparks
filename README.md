...
- **源代码**（包括主题文件、配置文件、脚本等）：基于 [MIT 许可证](./LICENSE) 开源。  
  （即保留了原模板作者 FromEndWorld 的版权信息，以及你的修改版权声明）

- **博客文章内容**（位于 `_posts` 目录下的所有 Markdown 文件）：采用 [知识共享署名-非商业性使用-相同方式共享 4.0 国际许可协议](https://creativecommons.org/licenses/by-nc-sa/4.0/)（CC BY-NC-SA 4.0）进行许可。  
  这意味着你可以自由分享、改编文章，但必须署名、不得用于商业目的，且衍生作品需采用相同协议。

源代码的详细许可请参阅根目录的 `LICENSE` 文件。

# 以下为源代码创作者内容” 本身就是一个一级标题
